...
const HISTORY_KEY = 'crypto_radar_live_history_v14';
const state = {
  tab: 'home', loading: false, data: null, started: false,
  lastErrors: [], autoTimer: null, ws: null, wsOk: false,
  liq: {
    startedAt: null,
    BTC: {longUsd:0, shortUsd:0, events:0},
    ETH: {longUsd:0, shortUsd:0, events:0}
  }
};
...
async function bybitRatio(asset){
  const symbol=`${asset}USDT`;
  const j=await fetchJson(`${BYBIT}/v5/market/account-ratio?category=linear&symbol=${symbol}&period=1h&limit=1`,{},5000);
  if(n(j?.retCode)!==0) throw new Error(j?.retMsg||'Bybit ratio error');
  const row=j?.result?.list?.[0];
  const longPct=n(row?.buyRatio); const shortPct=n(row?.sellRatio);
  if(longPct==null||shortPct==null) throw new Error('Bybit ratio missing');
  return {ratio:shortPct?longPct/shortPct:null,longPct:longPct*100,shortPct:shortPct*100,provider:'Bybit 1H'};
}
async function binanceRatio(asset){
  const pair=`${asset}USD`;
  const j=await fetchJson(`${BINANCE_COINM}/futures/data/globalLongShortAccountRatio?pair=${pair}&period=1h&contractType=PERPETUAL&limit=1`,{},5000);
  const row=Array.isArray(j)?j[0]:null;
  const longPct=n(row?.longAccount); const shortPct=n(row?.shortAccount);
  if(longPct==null||shortPct==null) throw new Error('Binance ratio missing');
  return {ratio:n(row?.longShortRatio),longPct:longPct*100,shortPct:shortPct*100,provider:'Binance COIN-M 1H'};
}
async function loadRatio(asset){
  try { return await bybitRatio(asset); } catch(e1) {
    try { return await binanceRatio(asset); } catch(e2) { return {error:`${e1.message}; ${e2.message}`}; }
  }
}
...
function resetLiquidationSession(){
  state.liq={startedAt:Date.now(),BTC:{longUsd:0,shortUsd:0,events:0},ETH:{longUsd:0,shortUsd:0,events:0}};
}
function connectLiquidations(){
  try { state.ws?.close(); } catch {}
  resetLiquidationSession(); state.wsOk=false;
  try {
    const ws=new WebSocket(BYBIT_WS); state.ws=ws;
    const kill=setTimeout(()=>{ if(ws.readyState!==WebSocket.OPEN){ try{ws.close();}catch{} } },7000);
    ws.onopen=()=>{
      clearTimeout(kill); state.wsOk=true;
      ws.send(JSON.stringify({op:'subscribe',args:['allLiquidation.BTCUSDT','allLiquidation.ETHUSDT']}));
      if(state.data) render();
    };
    ws.onmessage=ev=>{
      try {
        const m=JSON.parse(ev.data); const arr=Array.isArray(m?.data)?m.data:[];
        for(const x of arr){
          const sym=String(x?.s||''); const asset=sym.startsWith('BTC')?'BTC':sym.startsWith('ETH')?'ETH':null;
          if(!asset) continue;
          const qty=n(x?.v), price=n(x?.p); const usd=(qty!=null&&price!=null)?qty*price:null;
          if(usd==null) continue;
          // Bybit docs: S=Buy means a long position was liquidated; S=Sell means a short position was liquidated.
          if(x?.S==='Buy') state.liq[asset].longUsd+=usd;
          else if(x?.S==='Sell') state.liq[asset].shortUsd+=usd;
          state.liq[asset].events+=1;
        }
        if(state.tab==='market'||state.tab==='home'||state.tab==='settings') render();
      } catch {}
    };
    ws.onerror=()=>{state.wsOk=false;if(state.data)render();};
    ws.onclose=()=>{state.wsOk=false;if(state.started)setTimeout(()=>{if(state.started&&!document.hidden)connectLiquidations();},5000);if(state.data)render();};
  } catch { state.wsOk=false; }
}
...
async function refresh(){
  ...
  try {
    ...
    const [br,er,be,ee]=await Promise.allSettled([loadRatio('BTC'),loadRatio('ETH'),loadEtf('BTC'),loadEtf('ETH')]);
    const ratioB=br.status==='fulfilled'&&!br.value?.error?br.value:null;
    const ratioE=er.status==='fulfilled'&&!er.value?.error?er.value:null;
    const etfB=be.status==='fulfilled'?be.value:null; const etfE=ee.status==='fulfilled'?ee.value:null;
    ...
    const optErrors=[];
    if(!ratioB) optErrors.push(`BTC Long/Short: ${br.status==='fulfilled'?br.value?.error:br.reason?.message||br.reason}`);
    if(!ratioE) optErrors.push(`ETH Long/Short: ${er.status==='fulfilled'?er.value?.error:er.reason?.message||er.reason}`);
...
function hero(asset,m){
  return `<section class="hero"><div class="hero-top"><div><div class="coin">${asset} · ${escapeHtml(m.provider)}</div><div class="price">${money(m.price,'USD',0)}</div><div class="sub">24H <span class="delta ${clsDelta(m.price24)}">${pct(m.price24)}</span> · ${new Date(m.at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</div></div>${liveBadge()}</div>
    <div class="grid"><div class="metric"><div class="label">Open Interest</div><div class="value">${compact(m.oiUSD)}</div><div class="note">24H ${m.oi24==null?'收集中':`<span class="delta ${clsDelta(m.oi24)}">${pct(m.oi24)}</span>`}</div></div><div class="metric"><div class="label">Funding ${m.fundingPeriod||''}</div><div class="value">${m.funding==null?'—':m.funding.toFixed(4)+'%'}</div><div class="note">${fundingState(m)}</div></div></div></section>`;
}
function liqMini(asset){
  const l=state.liq[asset]; const since=state.liq.startedAt?new Date(state.liq.startedAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):'—';
  return `<div class="card"><div class="section-title"><h3>${asset} Liquidation</h3><small>${state.wsOk?'Bybit LIVE':'Disconnected'}</small></div><div class="grid"><div class="metric"><div class="label">Long liq</div><div class="value">${compact(l.longUsd)}</div></div><div class="metric"><div class="label">Short liq</div><div class="value">${compact(l.shortUsd)}</div></div></div><div class="fineprint">本 App 从 ${since} 开始收到的 Bybit 强平事件，共 ${l.events} 笔。不是全球 24H 总数。</div></div>`;
}
...
function marketCard(asset,m){
  ...
  return `<section class="section"><div class="section-title"><h3>${asset} Market Structure</h3><small>${escapeHtml(m.provider)}</small></div><div class="card"><div class="grid three">
    ...
    <div class="metric"><div class="label">Long liquidated</div><div class="value">${compact(l.longUsd)}</div><div class="note">Bybit since open</div></div>
    <div class="metric"><div class="label">Short liquidated</div><div class="value">${compact(l.shortUsd)}</div><div class="note">Bybit since open</div></div>
...
function renderMarket(){
  const d=state.data; content.innerHTML=`${marketCard('BTC',d.btcM)}${marketCard('ETH',d.ethM)}<section class="section"><div class="section-title"><h3>Cross-venue 24H Liquidation</h3><small>ByKaranteli embed</small></div><div class="card" style="padding:8px"><iframe src="https://bykaranteli.com/embed/liquidations" title="24h cross-venue liquidations" width="100%" height="300" loading="lazy" style="border:0;border-radius:12px;background:#07111f"></iframe></div></section><p class="fineprint">Long/Short 是账户比例，不是全市场 Long 金额 vs Short 金额。上面的 BTC/ETH Liquidation 数字为 App 打开后收到的 Bybit 实时事件；Cross-venue widget 用于补充过去 24H 的多交易所记录。ETF 为发行商文件推算值，可能存在覆盖缺口。</p>`;
}
function renderSettings(){
  ...
  content.innerHTML=`<section class="section"><div class="section-title"><h3>LIVE Source Status</h3>${liveBadge()}</div><div class="card">
    ...
    <div class="row"><span class="left">Liquidation stream</span><span class="right">${state.wsOk?'Bybit LIVE':'Disconnected'}</span></div>
  ...
  <button id="clearHistBtn" class="btn ghost full" style="margin-top:10px">清除本机 OI 历史</button><p class="fineprint">OI 24H 是这台手机自己累计的 LIVE OI 快照。Liquidation 只从本次 App 打开后累计。ETF 数据由 XOOMAR 从 ETF 发行商文件读取并计算，页面中已标明覆盖限制。</p></section>`;
...
function init(){
  ...
  $('#startLiveBtn').onclick=()=>{state.started=true;$('#startDialog').close();connectLiquidations();refresh();startAutoRefresh();};
  $('#startDialog').showModal();
  document.addEventListener('visibilitychange',()=>{if(!document.hidden&&state.started){if(!state.wsOk)connectLiquidations();if(state.data&&Date.now()-state.data.refreshedAt>90000)refresh();}});
