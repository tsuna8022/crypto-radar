...
const HISTORY_KEY = 'crypto_radar_live_history_v18';
const LUNO_KEY = 'crypto_radar_luno_credentials_v18';
const state = {
...
function lunoAuth(creds){ return 'Basic '+btoa(`${creds.keyId}:${creds.keySecret}`); }
async function lunoJson(path,creds,timeoutMs=7000){
  return fetchJson(LUNO+path,{headers:{Authorization:lunoAuth(creds),Accept:'application/json'}},timeoutMs);
}
...
async function getLunoPortfolio(creds){
  const balances=await lunoJson('/api/1/balance',creds,7500);
  const tasks=await Promise.allSettled([
    lunoJson('/api/1/listtrades?pair=XBTMYR&limit=1000&sort_desc=false',creds,7500),
    lunoJson('/api/1/listtrades?pair=ETHMYR&limit=1000&sort_desc=false',creds,7500),
    fetchJson(LUNO+'/api/1/ticker?pair=XBTMYR',{},6500),
    fetchJson(LUNO+'/api/1/ticker?pair=ETHMYR',{},6500)
  ]);
  const btcTrades=tasks[0].status==='fulfilled'?(tasks[0].value.trades||[]):[];
  const ethTrades=tasks[1].status==='fulfilled'?(tasks[1].value.trades||[]):[];
  const btcMYR=tasks[2].status==='fulfilled'?n(tasks[2].value.last_trade):null;
  const ethMYR=tasks[3].status==='fulfilled'?n(tasks[3].value.last_trade):null;
  const warnings=[];
  if(tasks[0].status!=='fulfilled') warnings.push('BTC trade history unavailable: '+(tasks[0].reason?.message||tasks[0].reason));
  if(tasks[1].status!=='fulfilled') warnings.push('ETH trade history unavailable: '+(tasks[1].reason?.message||tasks[1].reason));
  if(tasks[2].status!=='fulfilled') warnings.push('XBTMYR ticker unavailable');
  if(tasks[3].status!=='fulfilled') warnings.push('ETHMYR ticker unavailable');
  const rows=balances.balance||[];
  ...
  return {
    ...
    btcTrades:tradeViews('BTC',btcTrades),ethTrades:tradeViews('ETH',ethTrades),warnings,rawBalances:rows,at:Date.now()
  };
}
async function refreshLuno({silent=false}={}){
  ...
  try{
    state.luno=await getLunoPortfolio(state.credentials); state.lunoLastSync=Date.now(); state.lunoError=null;
    if(!silent) toast('Luno 只读数据已更新');
  }catch(e){
    const msg=String(e?.message||e);
    state.lunoError=/Failed to fetch|Load failed|NetworkError|Timeout/i.test(msg)?'Safari 无法直接读取 Luno 私有 API（可能是 CORS / 网络限制）：'+msg:'Luno 读取失败：'+msg;
    if(!silent) toast('Luno 更新失败');
...
function renderPortfolio(){
  if(!state.credentials&&!state.luno){
    content.innerHTML=`<div class="empty"><b>Luno 尚未连接</b><br><br>市场 LIVE 已完成。现在可以连接 Luno Read-only，只读取余额、staking 与成交记录。<br><br><button id="connectLunoBtn" class="btn primary">连接 Luno Read-only</button>${savedLunoKey()?'<br><br><button id="unlockLunoBtn" class="btn ghost">用 PIN 解锁已保存 Key</button>':''}</div>`;
  ...
  if(!state.luno){ content.innerHTML=`<div class="error">${escapeHtml(state.lunoError||'Luno 尚未读取成功')}</div><button id="retryLunoBtn" class="btn primary full">重试 Luno</button><button id="changeLunoBtn" class="btn ghost full" style="margin-top:10px">更换 Luno Key</button>`; $('#retryLunoBtn').onclick=()=>refreshLuno(); $('#changeLunoBtn').onclick=openLunoSetup; return; }
  const p=state.luno;
...
function renderSettings(){
  ...
  const lunoState=state.credentials?(state.luno?'Connected · Read-only':state.lunoLoading?'Connecting':'Key loaded'):has?'Saved · locked':'Not connected';
  content.innerHTML=`<section class="section"><div class="section-title"><h3>LIVE Source Status</h3>${liveBadge()}</div><div class="card">
    ...
    <div class="row"><span class="left">Saved encrypted key</span><span class="right">${has?'Yes':'No'}</span></div>
    <div class="row"><span class="left">Server</span><span class="right">None</span></div>
  ...
  <button id="connectSettingsBtn" class="btn ghost full">${state.credentials?'更换 Luno Key':'连接 Luno Read-only'}</button>${has&&!state.credentials?'<button id="unlockSettingsBtn" class="btn ghost full" style="margin-top:10px">用 PIN 解锁已保存 Key</button>':''}${has?'<button id="forgetLunoBtn" class="btn ghost full" style="margin-top:10px;color:#ff9ba8">清除本机 Luno Key</button>':''}
  <button id="clearHistBtn" class="btn ghost full" style="margin-top:10px">清除本机 OI 历史</button>
  <div class="notice warning">Luno 官方目前建议只查看资料可直接选择 <b>Read-only access</b>，并保持额外限制选项关闭；若用 Custom，核心需要 View balance + View orders，View transactions 可一并开启。这个 App 没有任何下单、Send 或 Withdraw 代码。</div>
  <p class="fineprint">API Secret 只在这台浏览器使用。若选择保存，会先用你的 PIN 经 PBKDF2 + AES-GCM 加密再写入本机 localStorage；PIN 不会保存。关闭页面后需要重新输入 PIN 解锁。若 Safari 阻止 Luno 私有 API 的跨域请求，App 会明确显示 CORS / Network 错误，不会把 Secret 发到第三方代理。</p></section>`;
  $('#connectSettingsBtn').onclick=openLunoSetup;
  const u=$('#unlockSettingsBtn'); if(u)u.onclick=openLunoUnlock;
  const f=$('#forgetLunoBtn'); if(f)f.onclick=()=>{ if(confirm('清除这台手机保存的 Luno API Key？')){ localStorage.removeItem(LUNO_KEY); state.credentials=null;state.luno=null;state.lunoError=null;render();toast('本机 Luno Key 已清除'); } };
  $('#clearHistBtn').onclick=()=>{localStorage.removeItem(HISTORY_KEY);toast('OI 历史已清除');};
}
function openLunoSetup(){
  $('#lunoKeyId').value='';$('#lunoKeySecret').value='';$('#lunoPin').value='';$('#lunoSave').checked=true;$('#lunoError').classList.add('hidden');$('#lunoDialog').showModal();
}
...
function init(){
  ...
  $('#lunoForm').addEventListener('submit',async e=>{
    ...
    const keyId=$('#lunoKeyId').value.trim(),keySecret=$('#lunoKeySecret').value.trim(),pin=$('#lunoPin').value,save=$('#lunoSave').checked;
    if(!keyId||!keySecret){err.textContent='请输入 Key ID 和 Key Secret';err.classList.remove('hidden');return;}
    if(save&&pin.length<6){err.textContent='要保存 Key，PIN 至少 6 位';err.classList.remove('hidden');return;}
    const creds={keyId,keySecret};
    try{
      await lunoJson('/api/1/balance',creds,8000);
      ...
      err.textContent=/Failed to fetch|Load failed|NetworkError|Timeout/i.test(msg)?'手机 Safari 无法直接读取 Luno 私有 API（可能是 CORS / 网络限制）：'+msg:'Luno 连接失败：'+msg;
  ...
  $('#unlockForm').addEventListener('submit',async e=>{
    ...
    try{
      ...
      await lunoJson('/api/1/balance',state.credentials,8000);
      $('#unlockDialog').close(); state.tab='portfolio'; render(); await refreshLuno();
    }catch(x){
      const msg=String(x?.message||x); err.textContent=/OperationError|decrypt/i.test(msg)?'PIN 不正确':'解锁后 Luno 测试失败：'+msg; err.classList.remove('hidden');
    }
  });
