...
const HISTORY_KEY = 'crypto_radar_live_history_v17';
const state = {
...
function renderSettings(){
  ...
  <button id="clearHistBtn" class="btn ghost full" style="margin-top:10px">清除本机 OI 历史</button><p class="fineprint">OI 24H 是这台手机自己累计的核心 venue LIVE OI 快照。Liquidation 优先读取 ByKaranteli 过去 24H recorded feed；若 JSON 失败，市场页保留官方 liquidation embed。Positioning 直接使用 ByKaranteli BTC / ETH LIVE embeds，保留 Binance / Bybit / OKX / taker 等不同定义，避免压成一个误导性的统一比例。ETF 数据由 XOOMAR 从 ETF 发行商文件读取并计算，页面中已标明覆盖限制。</p></section>`;
